	...
	public void execute(String[] args) {
		...
		try {
			if (sname.startsWith("pred")) {
				...
				/* Vectors to store the computed error values */
				...
				Date end_date   = df.parse("2010-03-01"); 
				...
				for(int i = 1; i <= minLength; i++){					
					if(cur.equals(end_date)){
					...
					else if ( cur.compareTo(start_date) >= 0 && cur.compareTo(end_date) <= 0 ) { //inside start/end date interval
					    	...
					    	double actual_read_10 = pdb.getLoad("raw", cal.addMinutesTo(cur, 10)); // 10 minutes ago, and so on...
...
//				for(int z=0;z<e60.size();z++){
//					System.out.format("e60[%d]: \t%f\n",z,e60.get(z));
//				}
